...
namespace CpuSchedulingWinForms
{
    public partial class CpuScheduler : Form
    {
        ...
        private void btnSRTF_Click(object sender, EventArgs e)
        {
            ...
            {
                // *** Call the actual SRTF algorithm logic ***
                Algorithms.srtfAlgorithm(txtProcess.Text); // This will handle its own popups

                // Keep the existing UI update logic (cosmetic)
                ...
                // Update ListView (Basic - just shows process count)
                // TODO: Modify this if Algorithms.srtfAlgorithm returns results
                //       to display actual WT, TAT, etc.
                listView1.Clear();
                listView1.View = View.Details;
                listView1.Columns.Add("Process ID", 150, HorizontalAlignment.Center);
                listView1.Columns.Add("Algorithm", 100, HorizontalAlignment.Center); // Indicate algorithm

                for (int i = 0; i < numberOfProcess; i++)
                {
                    var item = new ListViewItem();
                    item.Text = "Process " + (i + 1);
                    item.SubItems.Add("SRTF"); // Add algorithm type
                    listView1.Items.Add(item);
                }

                listView1.Items.Add(""); // Add a blank line for spacing
                ListViewItem summaryItem = new ListViewItem("Handles:");
                summaryItem.SubItems.Add(numberOfProcess.ToString());
        ...
        private void btnMLFQ_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtProcess.Text, out int numberOfProcess) && numberOfProcess > 0)
            {
                // calls the algorithim and handles pop up windows
                ...
                // Update ListView (Basic - just shows process count)
                // TODO: Modify this if Algorithms.mlfqAlgorithm returns results
                //       to display actual WT, TAT, Final Queue etc.
                listView1.Clear();
                listView1.View = View.Details;
                listView1.Columns.Add("Process ID", 150, HorizontalAlignment.Center);
                listView1.Columns.Add("Algorithm", 100, HorizontalAlignment.Center); // Indicate algorithm
                ...
                for (int i = 0; i < numberOfProcess; i++)
                {
                    var item = new ListViewItem();
                    item.Text = "Process " + (i + 1);
                    item.SubItems.Add("MLFQ"); // Add algorithm type
                    listView1.Items.Add(item);
                }
